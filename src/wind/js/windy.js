export const Windy = function (params) {
  ...
  var createWindBuilder = function (u, v, header) {
    return {
      header,
      data: function (i) {
        return [u[i], v[i]];
  ...
  var createBuilder = function (data) {
    return createWindBuilder(data.values.u, data.values.v, data.json.header);
  };
  ...
  var buildGrid = function (data, callback) {
    ...
    if (!supported)
      console.log('Error: Data must contain a json and a png file');

    // check if json or png is needed
    builder = createBuilder(data);
    var header = data.json.header;
